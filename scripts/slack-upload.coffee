...
#
# Commands:
#   hubot show me the applicants for job <id>
#   hubot show me the referral (refCode|%) in (id|sg) from <YYYY-MM-DD> to <YYYY-MM-DD>
#   hubot show me the weekly stats of (id|sg)
#
# Author:
#   Ying Cong
...
module.exports = (robot) ->
  ...
  robot.hear /show me the referral (\S+)(?: in (sg|id))?(?: from ((?:\d|\-)+) to ((?:\d|\-)+))?/i, (res) ->
    ...
    if !country or !startDate or !endDate
      res.send ":japanese_ogre: Please indicate the country. `show me the referral <refCode> in <sg or id> from <YYYY-MM-DD> to <YYYY-MM-DD>`. But out of the kindness of my metal heart, I'm assuming Indonesia and forever"
      country = 'id'
